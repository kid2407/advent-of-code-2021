...
const INPUT_FILE = '../inputs/day4.txt';
...
while (!$winningBoardFound) {
    ...
    foreach ($bingoFields as $fieldNumber => $bingoField) {
        if (checkIfBoardHasWon($bingoField)) {
            $unmarkedSum = getSumOfUnmarkedNumbersOfBoard($bingoField);
            echo sprintf("Sum for Board %d: %d%s", $fieldNumber, $unmarkedSum, PHP_EOL);
